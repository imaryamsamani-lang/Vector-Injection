...
flags.DEFINE_string('ckpt', "gemma-2b.ckpt", 'Path to the checkpoint file.') #None, 'Path to the checkpoint file.', required=True)
...
def main(_):
    ...
    import time
    t = time.time()
    result = model.generate(FLAGS.prompt,
                            device, output_len=FLAGS.output_len)
...
# # python scripts/run.py --device=cpu --ckpt=/path/to/your/pytorch_checkpoint/model.ckpt --output_len=128 --prompt="The name of the capital of Italy is"
# # Important:
